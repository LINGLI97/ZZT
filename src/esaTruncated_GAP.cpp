...
static inline vector<INT> merge_sorted_occ(const vector<INT>& a,
                                           const vector<INT>& c) {
    vector<INT> r;
    r.reserve(a.size() + c.size());
    size_t i = 0, j = 0;
    while (i < a.size() && j < c.size()) {
        if (a[i] <= c[j]) r.push_back(a[i++]);
        else              r.push_back(c[j++]);
    }
    while (i < a.size()) r.push_back(a[i++]);
    while (j < c.size()) r.push_back(c[j++]);
    return r;
}
static inline long long sum_gaps_from_sorted_occ(const vector<INT>& occ) {
    if (occ.size() <= 1) return 0LL;
    return (long long)occ.back() - (long long)occ.front();
}


inline void absorb_child(INT parent, INT child, vector<B>& b) {
    // Merge occ in sorted order
    if (b[parent].occ.empty()) {
        b[parent].occ.swap(b[child].occ);
    } else if (!b[child].occ.empty()) {
        auto merged = merge_sorted_occ(b[parent].occ, b[child].occ);
        b[parent].occ.swap(merged);
        vector<INT>().swap(merged);
        vector<INT>().swap(b[child].occ);
    }


    b[parent].gap_sum = sum_gaps_from_sorted_occ(b[parent].occ);
...
// Special handling for zigzag gap_sum
...
//                    b[parent].ch.push_back(leaf_idx);
                    absorb_child(parent, leaf_idx, b);
                }
