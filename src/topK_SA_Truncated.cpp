...
// ---- Direct comparison versions (no SA/LCE needed, only T and B) ----
...
bool isZLess_direct(INT i, INT j, unsigned char* T, INT text_size, INT B) {
    ...
    return getZigZagChar(i, d, T, text_size, B)
         <= getZigZagChar(j, d, T, text_size, B);
}
...
void mergeSortIterativeZigZag_direct(INT* indices, unsigned char* T, INT text_size,
                                     INT* LCP, INT B) {
    vector<INT> temp(text_size);
...
//    unsigned char *text_string_woMax;
    ...
    // A full lexicographic order keeps each Bound-prefix class contiguous;
    // clamping ZZLCP therefore gives the same truncated prefix intervals.
#elif defined(USE_DIRECT_COMPARE)
    // Direct comparison: only needs T and B, no SA/LCE structures
    mergeSortIterativeZigZag_direct(SA_Z, textStringWoMax, text_size, LCP_Z, Bound);
#else
    // LCE-based: builds full SA+RMQ on T and rev(T)
    {
    ...
    {
        int_vector<> v(text_size, 0);
        for (INT i = 0; i < text_size; i++) v[i] = LCP_Z[i];
    ...
    unordered_map<uint64_t, uint64_t> id2right;
#ifdef USE_3D
...
//id2right: from id to each rightmost id
        for (int i = 0; i < b.size(); i++) {

            if (b[i].id>0){
                id2right[b[i].id] = b[b[i].right_node].id;
     ...
     for (int i = 0; i < b.size(); i++) {
       cout << "i=" << i << ", l=" << b[i].l << ", r=" << b[i].r << ", lcp=" << b[i].lcp  <<", id = "<< b[i].id << ", parent=" << b[i].parent <<", # of children:"<< b[i].leaf_cnt <<", rightmost leaf id: "<<b[i].right_node <<", children are: "<<endl;
...
#ifdef USE_3D
    bgi::rtree<scored_point3, bgi::rstar<8>> RT(points);
#else
    bgi::rtree<point4, bgi::rstar<8>> RT(points);
#endif
...
//        cout << "Interval: [" << intervalP.first << ", " << intervalP.second << "]" << endl;
        ...
        uint64_t rightpreorderId = id2right[id_P];
...
//        next_greater_id_with_right(id_P, id2right, next_id, rightpreorderId);
        ...
        bg::set<1>(min_point, static_cast<uint64_t>(patternSizes[i] + 2 * contextSize));
        bg::set<2>(min_point, static_cast<uint64_t>(0));
        bg::set<3>(min_point, static_cast<uint64_t>(0));
        bg::set<0>(max_point, static_cast<uint64_t>(rightpreorderId));
        bg::set<1>(max_point, static_cast<uint64_t>(INT_MAX));
        bg::set<2>(max_point, static_cast<uint64_t>(patternSizes[i] + 2 * contextSize));
        bg::set<3>(max_point, static_cast<uint64_t>(intervalP.second - intervalP.first + 1));
...
#ifdef USE_3D
        // score is attached as cand[j].second — no map lookup
        scores.resize(m);
        for (INT j = 0; j < m; j++)
            scores[j] = cand[j].second;
        ...
        double t_generateZP= std::chrono::duration_cast<std::chrono::microseconds>(time2 - time1).count() * 0.000001;
        double t_patmatch  = std::chrono::duration_cast<std::chrono::microseconds>(time3 - time2).count() * 0.000001;
        double t_id        = std::chrono::duration_cast<std::chrono::microseconds>(time4 - time3).count() * 0.000001;
        double t_id2right  = std::chrono::duration_cast<std::chrono::microseconds>(time34 - time4).count() * 0.000001;
        double t_rtquery   = std::chrono::duration_cast<std::chrono::microseconds>(time5 - time34).count() * 0.000001;
        double t_collect   = std::chrono::duration_cast<std::chrono::microseconds>(time6 - time5).count() * 0.000001;
        double t_nth       = std::chrono::duration_cast<std::chrono::microseconds>(time7 - time6).count() * 0.000001;
        ...
        cout << "  generateZP:    " << t_generateZP << "s\n";
        cout << "  pattern_match: " << t_patmatch   << "s\n";
        cout << "  compute id_P:  " << t_id         << "s\n";
        cout << "  id2right:      " << t_id2right   << "s\n";
        cout << "  RT.query:      " << t_rtquery    << "s\n";
#ifdef USE_3D
        cout << "  collect score: " << t_collect    << "s\n";
#endif
        cout << "  nth_element:   " << t_nth        << "s\n";
        cout << "  Total query:   " << query_time   << "s\n";
