...
//    unsigned char *text_string_woMax;
    ...
    // Full order plus clamped ZZLCP represents the Bound-prefix intervals.
    {
    ...
    //from  id to right most id and sumGap
    unordered_map<uint64_t , pair<uint64_t,INT>> id2rightGap;
#ifdef USE_3D
...
//id2right: from id to each rightmost id
        for (int i = 0; i < b.size(); i++) {
            ...
            if (b[i].id>0){
                id2rightGap[b[i].id] = {b[b[i].right_node].id, b[i].gap_sum};
...
// Step 4: build R-tree
    ...
    double Construction_time = std::chrono::duration_cast<std::chrono::microseconds>(t_end - start).count() * 0.000001;
...
//        cout << "Interval: [" << intervalP.first << ", " << intervalP.second << "]" << endl;
        ...
        uint64_t rightpreorderId = id2rightGap[id_P].first;
        INT sumGap = id2rightGap[id_P].second;
...
//        next_greater_id_with_right(id_P, id2right, next_id, rightpreorderId);
        ...
        bg::set<1>(max_point, static_cast<uint64_t>(INT_MAX));
        bg::set<2>(max_point, static_cast<uint64_t>(patternSizes[i] + 2 * contextSize));
        bg::set<3>(max_point, static_cast<uint64_t>(sumGap));
