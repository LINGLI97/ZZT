...
/*Given P and context length l, use RMQ function to partition the intervals*/
        ...
        for (pair<INT, INT> subInterval: subIntervalsRMQ) {
            ...
            const auto rpos = rmq_C(subInterval.first, subInterval.second);
            const INT sa_rev_val = SA_rev[rpos];
            INT indexFirst = n - patternSizes[i] - x - sa_rev_val;
            //            INT indexSecond = n - pattern_size - l  - SA_rev[rmq_C(subInterval.first,subInterval.second)];

            if (indexFirst < n and indexFirst >= 0) {
        ...
        for (auto subInterval: IntervalsMapped) {
            ...
            for (pair<INT, INT> subsubInterval: subIntervalsRMQNormal) {
                ...
                const INT base = n - patternSizes[i]- x - y;
                const auto pos  = subsubInterval.first;
        ...
        vector<pair<INT, INT>> frequency_TP;
        for (auto interval : subIntervalsRMQNonNeg) {
            INT minGap = INT_MAX;
            INT tmpGap = 0;
            vector<INT> GapList;
            if (interval.first == interval.second) {
                tmpGap = INT_MAX;
                GapList.push_back(tmpGap);
            } else{

                for (INT j = interval.first+1; j < interval.second + 1; ++j) {
                    tmpGap = abs(SA[j] - SA[j-1]);
                    GapList.push_back(tmpGap);
                }
            ...
            minGap = GapList[0];
            frequency_TP.push_back({minGap,interval.second-interval.first+1});;
        }
        ...
        if (bottomMode) {
            ...
            if(frequency_TP.size()>k){
                ...
                std::cout << "We can get bottom " << kSizes[i] << " from " << frequency_TP.size() << " results." << endl;

            } else if (frequency_TP.size()<k){

                for (int xx = 0; xx < frequency_TP.size(); ++xx) {

                }
                std::cout << "Pattern " << i << ": " << patterns[i] << std::endl;
                std::cout << "l: " <<contextSizes[i].first <<"; r: " <<contextSizes[i].second << "; k: " << k << std::endl;

                std::cout << "We can get only get bottom " << frequency_TP.size() << " though k = " << k << endl;
            ...
            std::sort(frequency_TP.begin(), frequency_TP.end(),
            ...
            if(frequency_TP.size()>k){
                ...
                std::cout << "We can get top " << kSizes[i] << " from " << frequency_TP.size() << " results." << endl;
                ...
                std::cout << "We can get only get top " << frequency_TP.size() << " though k = " << k << endl;
