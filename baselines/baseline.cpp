...
/*Given P and context length l, use RMQ function to partition the intervals*/
        ...
        for (const auto& interval : subIntervalsRMQNonNeg) {
            frequency.push_back(interval.second - interval.first + 1);
        }
        ...
        if (bottomMode) {
            ...
            if(frequency.size()> k){
                ...
                std::cout << "We can get bottom " << kSizes[i] << " from " << frequency.size() << " results." << endl;

            } else if (frequency.size()< k){

                for (int xx = 0; xx < frequency.size(); ++xx) {

                }
                std::cout << "Pattern " << i << ": " << patterns[i] << std::endl;
                std::cout << "l: " <<contextSizes[i].first <<"; r: " <<contextSizes[i].second << "; k: " << k << std::endl;
                std::cout << "We can get only get bottom " << frequency.size() << " though k = " << k << endl;
            ...
            if(frequency.size()>k){
                ...
                std::cout << "We can get top " << kSizes[i] << " from " << frequency.size() << " results." << endl;
                ...
                std::cout << "We can get only get top " << frequency.size() << " though k = " << k << endl;
